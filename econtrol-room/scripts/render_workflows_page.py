...
def health_card(label: str, value: str, note: str, tone: str = 'neutral') -> str:
    return f'''<article class="health-card tone-{escape(tone)}"><span class="health-label">{escape(label)}</span><strong class="health-value">{escape(value)}</strong><div class="health-note">{escape(note)}</div></article>'''
...
def main() -> None:
    ...
    overall = str(health.get('overall_status', 'unknown')).lower()
    overall_tone = 'good' if overall in {'healthy', 'ok', 'success'} else 'warn' if overall in {'warning', 'degraded', 'partial'} else 'danger' if overall in {'blocked', 'failed', 'error'} else 'neutral'
    health_cards = ''.join([
        health_card('Active', str(active_count), 'Workflows live', 'good'),
        health_card('Blocked', str(blocked_count), 'Vraagt aandacht', 'danger' if blocked_count else 'good'),
        health_card('Approvals', str(approval_count), 'Menselijke gates', 'warn' if approval_count else 'neutral'),
        health_card('Overall', str(health.get('overall_status', 'unknown')), 'Machine gezondheid', overall_tone),
    ])
    alerts_list = ''.join(f"<li class='alert-item tone-{"danger" if item.get("priority") == "high" else "warn" if item.get("priority") == "medium" else "neutral"}'><strong>{escape(item.get('type', 'alert'))}</strong><div class='sub'>{escape(item.get('message', ''))}</div></li>" for item in alerts) or '<li class="alert-item tone-good">Geen alerts.</li>'
    recommendation_list = ''.join(f"<li class='alert-item tone-{"warn" if item.get("priority") in {"high","medium"} else "neutral"}'><strong>{escape(item.get('title', '-'))}</strong><div class='sub'>{escape(item.get('recommended_action', ''))}</div></li>" for item in recommendations) or '<li class="alert-item tone-good">Geen aanbevelingen.</li>'
    ...
    .health-card.tone-good{background:rgba(34,197,94,.10);border-color:rgba(34,197,94,.26)}
    .health-card.tone-warn{background:rgba(245,158,11,.10);border-color:rgba(245,158,11,.28)}
    .health-card.tone-danger{background:rgba(239,68,68,.10);border-color:rgba(239,68,68,.30)}
    ...
    .tone-good .health-value{color:#86efac}
    .tone-warn .health-value{color:#fcd34d}
    .tone-danger .health-value{color:#fca5a5}
    ...
    .alert-item{list-style:none;padding:12px 14px;border-radius:14px;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.03)}
    .alert-item.tone-good{background:rgba(34,197,94,.08);border-color:rgba(34,197,94,.22)}
    .alert-item.tone-warn{background:rgba(245,158,11,.10);border-color:rgba(245,158,11,.24)}
    .alert-item.tone-danger{background:rgba(239,68,68,.10);border-color:rgba(239,68,68,.26)}
    @media (max-width:1180px){.health-grid,.workflow-grid,.action-grid,.meta-grid,.meta-grid-tight,.workflow-foot,.meta-foot{grid-template-columns:1fr}.workflow-card,.action-card,.meta-card{padding:14px;border-radius:16px}.workflow-card h3,.action-card h3,.meta-card h3{font-size:18px}.panel{padding:16px 14px 14px}.stack{gap:8px}}
    '''
