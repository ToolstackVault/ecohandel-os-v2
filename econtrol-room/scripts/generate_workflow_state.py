...
WEFACT_DATA_PATH = BASE / 'dashboard-data' / 'data' / 'wefact.json'
...
def build_registry(ops_status: dict, cron_status: dict, deploy_status: dict, publish_status: dict) -> list[dict]:
    ...
    wefact_data = load_json(WEFACT_DATA_PATH)
    wefact_live = str(wefact_data.get('status', '')).lower() == 'live'
    wefact_fetched_at = wefact_data.get('fetched_at')
    wefact_note = 'Read-only finance truth layer is live via Wefact API.' if wefact_live else 'Business-critical volgende laag voor omzetwaarheid.'
    wefact_error = None if wefact_live else 'Wefact koppeling is nog niet live of levert geen geldige status terug.'
    workflows = [
        {
            'id': 'ops_cycle',
        ...
        },
        {
            ...
            'lane': 'ingest',
            'description': 'Read-only finance truth layer voor facturen, offertes en omzetvalidatie.',
            'driver_type': 'script',
            'owner': common_owner,
            'mode': 'manual',
            'enabled': wefact_live,
            'approval_required': True,
            'status': 'live' if wefact_live else 'blocked',
            'last_run': wefact_fetched_at,
            'next_run': None,
            'dependencies': ['wefact'],
            'outputs': ['state/finance-status.json'],
            'last_error': wefact_error,
            'notes': wefact_note,
        },
        {
            'id': 'cron_health',
...
def build_dependencies(source_signals: dict, cron_status: dict) -> dict:
    ...
    items = [
        ...
        },
        {
            'id': 'wefact',
            'name': 'Wefact',
            'status': 'ready' if wefact_live else 'blocked',
            'freshness_hours': age_hours(wefact_data.get('fetched_at')),
