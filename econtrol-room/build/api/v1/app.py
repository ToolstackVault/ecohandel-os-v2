...
APP_ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # /var/www/html/control.ecohandel.nl
...
MILAN_PASSWORD = 'ecohandel2026'
TOM_PASSWORD = 'tom2026'
    ...
    def decorated(*args, **kwargs):
        ...
        if auth.startswith('Basic '):
            try:
                ...
                if username == 'milan' and password == MILAN_PASSWORD:
                    g.current_user = 'milan'
                    g.is_agent = False
                    print("[DEBUG AUTH] Milan OK", flush=True)
                    return f(*args, **kwargs)
                if username == 'tom' and password == TOM_PASSWORD:
                    g.current_user = 'tom'
                    g.is_agent = False
                    return f(*args, **kwargs)
...
def load_json_file(filename: str, default=None):
    """Load a JSON file from the dashboard-data/data directory."""
    import pathlib
    path = pathlib.Path(APP_ROOT_DIR) / 'dashboard-data' / 'data' / filename
    try:
        if path.exists():
...
@require_auth
def get_dashboard():
    """Return all dashboard KPIs, GSC, GA4, Brevo, WeFact data."""
    gsc     = load_json_file('gsc.json',     {})
    ga4     = load_json_file('ga4.json',     {})
    wefact  = load_json_file('wefact.json',  {})
    ads     = load_json_file('ads.json',     {})
    ...
    qh = query_db(
        """SELECT COUNT(*) as total,
                  SUM(CASE WHEN lane='top_5_now'   THEN 1 ELSE 0 END) as top5,
                  SUM(CASE WHEN lane='next_up'     THEN 1 ELSE 0 END) as next_up,
                  SUM(CASE WHEN lane='watchlist'   THEN 1 ELSE 0 END) as watchlist,
                  SUM(CASE WHEN lane='unqueued'    THEN 1 ELSE 0 END) as unqueued,
                  SUM(CASE WHEN lane='refresh_first' THEN 1 ELSE 0 END) as refresh,
                  SUM(CASE WHEN status='done'      THEN 1 ELSE 0 END) as done,
                  SUM(CASE WHEN priority_label='P1' THEN 1 ELSE 0 END) as p1,
                  SUM(CASE WHEN priority_label='P2' THEN 1 ELSE 0 END) as p2
    ...
    return api_response({
        ...
        'health':   health,
        'meta':     meta,
        'queue':    dict(qh) if qh else {},
        'agents':   recent_runs,
    })


@app.route('/workflows/history', methods=['GET'])
@require_auth
    ...
    if workflow_id:
        rows = query_db(
            """SELECT wr.*, w.name as workflow_name
               ...
               WHERE wr.tenant_id = ? AND wr.workflow_id = ?
               ORDER BY wr.started_at DESC LIMIT ?""",
            (g.tenant_id, workflow_id, min(limit, 200))
        )
    else:
        rows = query_db(
            """SELECT wr.*, w.name as workflow_name
               ...
               WHERE wr.tenant_id = ?
               ORDER BY wr.started_at DESC LIMIT ?""",
            (g.tenant_id, min(limit, 200))
        )
    return api_response({'runs': rows})
