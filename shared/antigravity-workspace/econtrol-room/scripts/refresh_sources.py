...
ECODASH_DIR = Path('/Users/ecohandel.nl/.openclaw/workspace/ecohandel/ecodash-v3/dashboard/data')
ECODASH_FETCH = Path('/Users/ecohandel.nl/.openclaw/workspace/ecohandel/ecodash-v3/scripts/fetch_all.py')
...
def maybe_refresh_ecodash() -> dict:
    if not ECODASH_FETCH.exists():
        return {'status': 'missing', 'detail': 'EcoDash fetch_all.py not found'}
    try:
...
def main() -> None:
    ...
    ecodash_refresh = maybe_refresh_ecodash()
    payload['ecodash_refresh'] = ecodash_refresh
    ...
    if ecodash_refresh.get('status') != 'ok':
        payload['warnings'].append('EcoDash live refresh did not complete cleanly; snapshot fallback used.')
